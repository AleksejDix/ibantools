/**
 * ISO 3166-1 alpha-2 codes of all countries known to the library, including countries without IBAN.
 * Kept separate from the country specifications so BIC validation does not bundle the IBAN data.
 * Every country in `ibanSpecs` must be listed here, because `countrySpecs` is built from this list.
 */
export const COUNTRY_CODES: ReadonlySet<string> = new Set(
  ...
  TR TT TV TW TZ UA UG UM US UY UZ VA VC VE VG VI VN VU WF WS XK YE YT ZA ZM ZW`.split(/\s+/u),
);
